...
class UnknownRunesAsm:
    ...
    # Opcode table
    OPCODES = {
        'HALT': 0x00, 'MOV': 0x01, 'MOVR': 0x02, 'ADD': 0x03, 'SUB': 0x04,
        'ADDI': 0x05, 'SUBI': 0x06, 'MUL': 0x07, 'DIV': 0x08, 'MOD': 0x09,
        'AND': 0x0A, 'OR': 0x0B, 'XOR': 0x0C, 'NOT': 0x0D, 'SHL': 0x0E,
        'SHR': 0x0F, 'LOAD': 0x10, 'STORE': 0x11, 'LOADI': 0x12, 'STOREI': 0x13,
        'JMP': 0x14, 'JEQ': 0x15, 'JNE': 0x16, 'JLT': 0x17, 'JGT': 0x18,
        'JLE': 0x19, 'JGE': 0x1A, 'MZERO': 0x1B, 'INC': 0x1C, 'DEC': 0x1D,
        'NEG': 0x1E, 'SYSCALL': 0x1F, 'PUSH': 0x20, 'POP': 0x21,
        'CALL': 0x22, 'RET': 0x23, 'PUSHI': 0x24, 'PUSHA': 0x25, 'POPA': 0x26,
    }
    ...
    FMTS = {
        ...
        'LOAD': 'rr',  'STORE': 'rr', 'LOADI': 'ri', 'STOREI': 'ri',
        'JMP': 'i',    'JEQ': 'rri',  'JNE': 'rri',  'JLT': 'rri',
        'JGT': 'rri',  'JLE': 'rri',  'JGE': 'rri',  'MZERO': 'r',
        'INC': 'r',    'DEC': 'r',    'NEG': 'r',    'SYSCALL': '*',
    ...
    def encInstr(self, op: int, r1: int = 0, r2: int = 0,
                 r3: int = 0, imm: int = 0) -> bytes:
    ...
    def pass2(self, src: str) -> bytearray:
        ...
        for ln, raw in enumerate(src.splitlines(), 1):
            ...
            if fmt == '*':
                ...
                if len(args) != len(fmt):
                    self.err(ln,
                             f"{mnem} expects {len(fmt)} operand(s), got {len(args)}")
                else:
