...
- If clientInfo is invalid (wrong magicPart/version) or does not include "Blake3" in supported hashes, server responds with 'IDK you' (0x52) and error code 0x0001 (Invalid Magic/Version) or 0x0002 (Hash Verification Failed) respectively.
...
- Reads key/value from global config store; if not found, returns 'IDK you' (0x52) with error code 0x0007 (Config Key Not Found).
